    ...
    <div className="min-h-screen bg-[#FDFBF7] grid md:grid-cols-5">
      <section className="hidden md:flex md:col-span-2 bg-[#1B4332] text-white relative overflow-hidden p-10">
        <div className="absolute inset-0 opacity-10">
          <svg viewBox="0 0 200 200" className="w-full h-full">
            <defs>
              <pattern id="islamic-register-pattern" width="40" height="40" patternUnits="userSpaceOnUse">
                <path d="M20 0L40 20L20 40L0 20Z" fill="none" stroke="currentColor" strokeWidth="1.2" />
              </pattern>
            </defs>
            <rect width="200" height="200" fill="url(#islamic-register-pattern)" />
          </svg>
        </div>
        <div className="relative z-10 max-w-sm self-end space-y-4">
          ...
          <p className="text-emerald-100">{t('registerSideText')}</p>
        </div>
      </section>
      ...
      <section className="md:col-span-3 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md space-y-6">
          <div>
            <h2 className="text-3xl font-bold text-emerald-950">{t('registerTitle')}</h2>
            <p className="text-emerald-900/60 mt-2">{t('registerSubtitle')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              <Globe className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-emerald-700/60" />
              <select
                value={country}
            ...
            <input
              ...
              className="w-full rounded-xl border border-emerald-900/20 bg-emerald-50 px-4 py-3 text-emerald-900/80"
            />
          ...
          <p className="text-sm text-emerald-900/70">
            {t('hasAccount')}{' '}
            <Link href="/login" className="text-emerald-900 font-semibold hover:underline">
              {t('signIn')}
            </Link>
          </p>
        </div>
